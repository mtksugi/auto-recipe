...
function renderDetail() {
    ...
    <div class="detail-body">
      <div class="detail-actions">
        ...
        <div class="recipe-management">
          <a class="edit-recipe-link" href="admin.html?edit=${encodeURIComponent(recipe.id)}">編集する</a>
          <button id="deleteRecipeButton" class="delete-recipe-button" type="button">削除する</button>
        ...
        ${wakeLockMessage || detailMessage ? `<p class="detail-action-message">${escapeHtml(wakeLockMessage || detailMessage)}</p>` : ""}
      </div>
  ...
  $("#deleteRecipeButton")?.addEventListener("click", () => deleteSelectedRecipe(recipe));
...
async function deleteSelectedRecipe(recipe) {
  if (!window.confirm(`「${recipe.title}」を削除します。\nこの操作は取り消せません。`)) return;
  const button = $("#deleteRecipeButton");
  ...
  try {
    ...
    if (!response.ok) throw new Error(data.error || "レシピを削除できませんでした");
    ...
    detailMessage = `${error.message} もう一度お試しください。`;
    renderDetail();
  }
}
